...
- Footer ต้องวัดได้จริง: `Tokens` จาก Tokens+Context policy, `Cache` จาก `Cache: X% hit`, `Session` จาก `Context: current/limit (percent%)`, `RTK` จากการใช้ RTK/command จริงในรอบนั้น
- search/excerpt/truncate ก่อน whole file เสมอ
- `RTK` → ลด token บน shell output 60-90%
- `โมเดลเล็ก: Yes` เฉพาะลด paid token หรือเวลาได้จริง; ไม่เข้าเงื่อนไขใช้ `โมเดลเล็ก: No`
