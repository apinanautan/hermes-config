...
|---|---|---|---|
| `Tokens` | `น้อย` / `ปกติ` / `สูง` / `สูงมาก` | `session_status` บรรทัด `Tokens` + `Context` ล่าสุด | คำนวณใหม่ทุกครั้ง; เทียบ policy ด้านล่าง; ถ้า input tokens กับ context% ให้คนละระดับ ให้ใช้ระดับที่สูงกว่า; ถ้า session ใหม่/compact แล้วลดลง ต้องลดระดับตามจริงทันที ห้ามค้างค่าเก่า |
...
⚠️ ห้าม carry forward ค่า Tokens/Cache/Session จากข้อความก่อนหน้าเมื่อเรียกค่าใหม่ได้ โดยเฉพาะ Tokens ต้อง auto ลด/เพิ่มตาม Context/Input ล่าสุด
